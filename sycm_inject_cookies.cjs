// 注入保存的 cookie 到 9225 浏览器并导航到生意参谋首页。
// 用法: node sycm_inject_cookies.cjs [cookie_json_path] [port]
const fs = require("fs");
const http = require("http");

const COOKIE_PATH = process.argv[2] || "/Users/luoxiaomin/.local/share/dashboard/sycm_cookies.json";
const PORT = process.argv[3] || 9225;
const HOME = "https://sycm.taobao.com/portal/home.htm";
...
http.get("http://localhost:" + PORT + "/json", (res) => {
  ...
  res.on("end", () => {
    ...
    sock.onopen = async () => {
      ...
      const r = await send("Network.setCookies", { cookies });
      const injected = cookies.length;
      ...
      console.log("INJECTED:" + injected + " HREF:" + href + " LOGIN:" + (loggedIn ? "OK" : "FAIL"));
      sock.close();
