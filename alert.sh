...
#   3) osascript 通知    兜底，仅交互式有效
#
# 为什么必须有它：launchd 后台任务用 osascript 发的通知会被 macOS 静默丢弃
# （exit=0 但不投递）——2026-09-13/09-14 巡检判定异常却无人知晓、数据停了 4 天
# 才被发现，根因就在这里。macOS 通知不可作主通道，真正的送达靠飞书。
#
...
# 3) macOS 通知：交互式调用有效；launchd 下会被系统丢弃，仅作兜底
